...
from bronze.extract.sances.pessoa import extrair_pessoa_sances
from bronze.extract.sults.pessoa import extrair_pessoa_sults
from silver.transform.pessoa.pessoa import transformar_pessoa

from repositories.pessoa_repository import (
    upsert_pessoa_sances_completo,
    upsert_pessoa_sults_completo,
...
class StepExtrairPessoaSances(Step):
    ...
    def execute(self, context: dict) -> dict:
        registros = extrair_pessoa_sances(
        ...
        resultado = upsert_pessoa_sances_completo(registros)
        context["bronze_sances_resultado"] = resultado
        context["bronze_sances_total"] = len(registros)
        context["tenant_id"] = self.tenant_id
        logger.info(f"[BRONZE-SANCES] tenant={self.tenant_id} {resultado}")
        return context


class StepExtrairPessoaSults(Step):
    """Bronze: extrai pessoas da API Sults e grava em pessoa_sults_raw."""

    def __init__(self, tenant_id: int, token: str, offset_inicial: int | None = None):
    ...
    def execute(self, context: dict) -> dict:
        registros = extrair_pessoa_sults(
            tenant_id=self.tenant_id,
            token=self.token,
            limit=100,
            offset_inicial=self.offset_inicial,
        ...
        resultado = upsert_pessoa_sults_completo(registros)
        context["bronze_sults_resultado"] = resultado
        context["bronze_sults_total"] = len(registros)
        context["tenant_id"] = self.tenant_id
        logger.info(f"[BRONZE-SULTS] tenant={self.tenant_id} {resultado}")
        return context


class StepTransformarPessoa(Step):
    """Silver: lê raw dos dois sistemas, une por CPF/CNPJ e grava em pessoa_bi."""
...
def executar_pipeline_pessoa(
    ...
    token_sances = buscar_token_por_nome("SANCES_TOKEN")
    token_sults = buscar_token_por_nome("SULTS_TOKEN")
...
def executar_todos_tenants() -> list[dict]:
    ...
    for t in tenants:
        logger.info(f"Iniciando pipeline de pessoa para tenant: {t['nome']} (id={t['id']})")
        try:
