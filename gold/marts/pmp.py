...
from repositories.financeiro_repository import buscar_bi_para_pmp_gold, upsert_pmp_gold

logger = get_layer_logger("gold", "pmp")


def processar_pmp() -> dict:
    """
    Lê financeiro_bi (registros RECEBER)
    e grava na tabela pmp_gold.
    """
    logger.info("Iniciando processamento da camada Gold — Prazo medio Pagamento")

    registros_bi = buscar_bi_para_pmp_gold()
    logger.info(f"Registros BI encontrados: {len(registros_bi)}")
    if not registros_bi:
        logger.info("Nenhum registro encontrado para prazo medio de pagamento.")
    ...
    for bi in registros_bi:
        try:
            gold = {
                ...
                "dias_pagamento":           bi.get("dias_pagamento"),
            }
            gold_registros.append(gold)
        except Exception as e:
            logger.error(f"Erro ao montar gold id_pessoa={bi.get('id_pessoa')}: {e}")

    resultado = upsert_pmp_gold(gold_registros)

    logger.info(f"Gold prazo medio pagamento: {resultado}")
